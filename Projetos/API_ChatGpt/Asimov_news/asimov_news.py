...
class AsimovNews:
    def __init__(self):
        ...
        self.sites = self._read_file(
            'sites') if 'sites' in os.listdir() else []
        ...
        while True:
            ...
            match self.screen:
                case 0:
                    ...
                    self.screen = int(self._receive_command(
                        ['1', '2', '3', '4'], 5))
                ...
                case 1:
                    ...
                    match command:
                        ...
                        case 'l':
                            link = int(
                                input('>> Insira o numero da materia que deseja abrir: '))
                ...
                case 2:
                    os.system('cls' if os.name == 'nt' else 'clear')
                    print(
                        'Digite o numero do site que deseja adicionar para a lista de sites ativos.\nPressione 0 para voltar para o menu.')
                    print("\tSITES ATIVOS ===============\n")
                    for i in self.sites:
                        print('\t', i)
                    ...
                    site = int(self._receive_command(
                        [str(i) for i in range(len(offline_sites)+1)], 50))
                ...
                case 3:
                    os.system('cls' if os.name == 'nt' else 'clear')
                    print(
                        'Digite o numero do site para remove-lo. Caso queira voltar para o Menu, digite 0\n')
    ...
    def display_news(self):
        ...
        for i, article in enumerate(self.filtered_news[constante:constante+10]):
            print(
                f"{constante+i+1}. {article['data'].strftime('%d/%m/%Y %H:%M')} - {article['fonte'].upper()} - {article['materia']}")
            ...
            for site in self.all_sites:
                ...
                for key, value in self.dict_site[site].news.items():
                    ...
                    if add_news:
                        self.news.insert(0, dict_aux)
            self.news = sorted(
                self.news, key=lambda d: d['data'], reverse=True)
            self._update_file(self.news, 'news')
            time.sleep(10)
