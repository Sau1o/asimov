...
def transcreve_tab_mic():
    prompt_mic = st.text_input(
        '(opcional) Digite o seu prompt', key='input_mic')
...
#         '(opcional) Digite o seu prompt', key='input_video')
#     arquivo_video = st.file_uploader(
#         'Adicione um arquivo de vídeo .mp4', type=['mp4'])
...
def transcreve_tab_audio():
    prompt_input = st.text_input(
        '(opcional) Digite o seu prompt', key='input_audio')
...
def main():
    ...
    st.markdown(
        '#### Transcreva áudio do microfone ou de arquivos de áudio')
